            ...
            try {
                await new Promise<void>((resolve, reject) => {
                    stream.on("data", (buffer: Buffer) => {
                        // if (this._status !== DownloaderStatus.RUNNING) {
    ...
    progressUpdateHandler(length: number, workerId: string): void {
        ...
        if (this.progressUpdateInterval === undefined) {
            ...
            this.progressCallback(
                this,
                this.stats,
                this.bytesSinceLastProgressUpdate,
            );
            return;
        }
        ...
        if (now - this.lastProgressUpdateTimestamp >= this.progressUpdateInterval) {
            ...
            if (this.progressCallback)
                this.progressCallback(this, this.stats, length);
            this.bytesSinceLastProgressUpdate = 0;
            ...
            if (this.logger && this.debugOutput) {
                ...
                this.workers.forEach((data) => {
                    ...
                    if (data.range) {
                        const rangeRemaining = data.range
                            ? data.range.end - data.range.start
                            : 0;
                        const percentage = (
                            (data.range.downloadedBytes / rangeRemaining) *
                            100
                        ).toFixed(0);
